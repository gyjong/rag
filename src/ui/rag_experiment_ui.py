...
from ..graphs.naive_rag_graph import create_naive_rag_graph, create_naive_rag_stream_graph
from ..graphs.advanced_rag_graph import create_advanced_rag_graph, create_advanced_rag_stream_graph
...
class RAGExperimentUI:
    ...
    def _display_experiment_interface(is_ready: bool):
        ...
        with st.container(border=True):
            ...
            with col1:
                if st.checkbox("Stream", value=False):
                    st.session_state.do_stream = True
                else:
                    st.session_state.do_stream = False
            ...
            try:
                with st.spinner(f"{system_name} 실행 중... 답변을 생성하고 있습니다."):
                    if system_name == "Naive RAG":
                        ...
                        do_stream = st.session_state.get("do_stream", False)
                        if do_stream:
                            result = RAGExperimentUI._stream_naive_rag(query, retrieval_k, llm_manager, vector_store_manager)
                        else:
                        ...
                        do_stream = st.session_state.get("do_stream", False)
                        if do_stream:
                            result = RAGExperimentUI._stream_advanced_rag(query, retrieval_k * 2, RERANK_TOP_K, llm_manager, vector_store_manager)
                        else:
    ...
    def _run_advanced_rag(query: str, k: int, rerank_top_k: int, llm_manager: LLMManager, vector_store_manager: VectorStoreManager) -> Dict[str, Any]:
        ...
        start_time = time.time()
        final_state = advanced_rag_graph.invoke(inputs, config=runnable_config)
        total_time = time.time() - start_time

        # 1. 쿼리 전처리 결과 표시
        st.subheader("🔧 1단계: 쿼리 전처리")
        preprocessing_details = final_state.get("preprocessing_details", {})
        optimized_query = final_state.get("optimized_query", "")

        if optimized_query != query:
            st.success(f"쿼리 최적화 완료")
            with st.expander("쿼리 확장 상세 정보 보기"):
                st.write(f"- **원본 쿼리:** `{preprocessing_details.get('original_query', '')}`")
                st.write(f"- **선택된 확장 용어:** `{preprocessing_details.get('selected_terms', '')}`")
        ...
        if reranked_docs:
            with st.expander(f"검색된 문서 ({len(reranked_docs)}개)"):
                for i, doc in enumerate(reranked_docs):
                    st.write(f"**문서 {i+1} (출처: {doc.metadata.get('source', 'Unknown')})**")
                    st.text(doc.page_content[:200] + "...")
                    st.divider()
        ...
        return {
            ...
            "rag_type": system_info["name"],
            "metadata": {
                "initial_retrieved": len(final_state.get("docs_with_scores", [])),
    ...
    @staticmethod
    def _stream_advanced_rag(query: str, k: int, rerank_top_k: int, llm_manager: LLMManager, vector_store_manager: VectorStoreManager) -> Dict[str, Any]:
        """Run the Advanced RAG in streaming mode and return a generator."""
        advanced_rag_graph = create_advanced_rag_stream_graph(llm_manager, vector_store_manager)

        inputs = {"query": query, "k": k, "rerank_top_k": rerank_top_k}
        runnable_config = {"callbacks": [langfuse_handler], "configurable": {"do_stream": True}}

        answer_generator = advanced_rag_graph.stream(inputs, config=runnable_config, stream_mode=["custom", "updates"])
        ...
        for chunk in answer_generator:
            if chunk[0] == "updates":
                ...
                if "preprocess_query" in updated_state:
                    # 1. 쿼리 전처리 결과 표시
                    st.subheader("🔧 1단계: 쿼리 전처리")
                    node_state = updated_state["preprocess_query"]
                    preprocessing_details = node_state["preprocessing_details"]
                    optimized_query = node_state["optimized_query"]
                ...
                elif "retrieve" in updated_state:
                    # 2. 문서 검색 결과
                    st.subheader("🔍 2단계: 문서 검색")
                    docs_with_scores = updated_state["retrieve"]["docs_with_scores"]
                    st.success(f"초기 검색: {len(docs_with_scores)}개 문서")

                elif "rerank" in updated_state:
                    # 3. 문서 재순위화 결과
                    st.subheader("📊 3단계: 문서 재순위화")
                    reranked_docs = updated_state["rerank"]["reranked_docs"]
                    st.success(f"재순위화 완료: 상위 {len(reranked_docs)}개 문서 선택")
                ...
                elif "compress_context" in updated_state:
                    # 4. 컨텍스트 압축 결과
                    st.subheader("🗜️ 4단계: 컨텍스트 압축")
                    compression_ratio = updated_state["compress_context"]["compression_ratio"]
                    st.info(f"압축률: {compression_ratio:.2%}")
                ...
                accumulated_answer += chunk[1]
                answer_placeholder.info(accumulated_answer)
    ...
    def _run_naive_rag(query: str, k: int, llm_manager: LLMManager, vector_store_manager: VectorStoreManager) -> Dict[str, Any]:
        ...
        start_time = time.time()
        final_state = naive_rag_graph.invoke(inputs, config=runnable_config)
        total_time = time.time() - start_time
    ...
    @staticmethod
    def _stream_naive_rag(query: str, k: int, llm_manager: LLMManager, vector_store_manager: VectorStoreManager) -> Dict[str, Any]:
        """Run the Naive RAG in streaming mode and return a generator."""
        naive_rag_graph = create_naive_rag_stream_graph(llm_manager, vector_store_manager)

        inputs = {"query": query, "k": k}
        runnable_config = {"callbacks": [langfuse_handler], "configurable": {"do_stream": True}}
        answer_generator = naive_rag_graph.stream(inputs, config=runnable_config, stream_mode=["custom", "values"])
